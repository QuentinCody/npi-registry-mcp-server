...
import { NpiRegistryDataDO } from "./do";

export { NpiRegistryDataDO };
...
export class MyMCP extends StatelessMcpWorker {
    ...
    async init() {
        ...
        const env = this.env as unknown as NpiEnv;
...
export default {
    fetch(request: Request, env: Env, ctx: ExecutionContext) {
        ...
        if (url.pathname === "/health") {
            return buildHealthResponse("npi-registry");
        }
        ...
        if (url.pathname === "/readyz") {
            ...
            return MyMCP.readiness(env, "npi-registry");
        }
