...
interface QueryEnv {
    NPI_REGISTRY_DATA_DO?: unknown;
}

export function registerQueryData(server: McpServer, env?: QueryEnv): void {
    const handler = createQueryDataHandler("NPI_REGISTRY_DATA_DO", "npi");

    server.registerTool(
        "npi_query_data",
        {
            title: "Query Staged NPI Registry Data",
            description:
                "Query staged NPPES NPI Registry data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
            inputSchema: {
